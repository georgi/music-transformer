...
class SequenceDataset(IterableDataset):
    """
    The purpose of the `SequenceDataset` class is to load, process,
    ...
    def __init__(self, sequence_files: List[str], seq_length: int, max_iter: int = -1):
    ...
    def __iter__(self):
        ...
        if worker_info is None:  # single-process data loading, return the full iterator
            ...
            per_worker = int(
                math.ceil(len(self.sequence_files) / float(worker_info.num_workers))
            )
